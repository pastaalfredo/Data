...
def save_table_as_latex(table, output_file, log_path):
    ...
    with open(output_file, 'w') as file:
        ...
        file.write(f"\\caption{{Parameters extracted from training log: {caption_label}.}}\n")
    ...
    for file in files:
        if file.endswith('.log'):
            ...
            if table:
                ...
                output_file = os.path.join(output_dir, f"{os.path.basename(root)}_{file.replace('.log', '')}.tex")
