...
gbs_file = "def2-TZVPP.gbs"

if not os.path.exists(gbs_file):
    raise FileNotFoundError(f"File '{gbs_file}' not found in current directory!")
...
for i in range(npoints):
    ...
    shutil.copy(gbs_file, os.path.join(tag, gbs_file))
...
with open("{tag}.out", "w") as result:
    result.write("    1\\n")
    result.write(" K 0.0 0.0 0.0\\n")
    result.write("    1\\n")
    result.write(" Br 0.0 0.0 {r:.16f}\\n")
